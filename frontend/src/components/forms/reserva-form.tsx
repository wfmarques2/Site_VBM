...
import { buildReservaMessage } from "@/services/whatsapp/build-reserva-message";
...
export function ReservaForm() {
  const initialDestino = useMemo(() => {
    const value = new URLSearchParams(window.location.search).get("destino");
    return value ? decodeURIComponent(value) : "";
  }, []);

  const initialOrigem = useMemo(() => {
    const value = new URLSearchParams(window.location.search).get("origem");
    return value ? decodeURIComponent(value) : "";
  }, []);
  ...
  const onSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const message = buildReservaMessage(parsed.data);
    setTimeout(() => {
